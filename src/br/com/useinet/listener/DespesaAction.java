...
public class DespesaAction implements ActionListener {
	...
	public void actionPerformed(ActionEvent actionEvent) {
		...
		if (actionEvent.getActionCommand().equals("Incluir")) {
			...
			if (despesaDb.criaObjeto(nomeDespesa, tipo, valor.toString()) == null) {
				JOptionPane.showMessageDialog(despesaUI, "Despesa já cadastrada.", "Registro existente",
		...
		if (linhaSelecionada == -1) {
			JOptionPane.showMessageDialog(despesaUI, "Selecione uma despesa.", "Selecionar registro",
		...
		try {
			...
			JOptionPane.showMessageDialog(despesaUI, "Tipo não encontrado.", "Registro inexistente",
					JOptionPane.WARNING_MESSAGE);
			despesaUI.getJComboBox().setModel(new TipoJComboBox().getModel());
			return;
		}
		...
		if ((despesa = (Despesa) despesaDb.listaObjeto(despesaNome, tipoVo)) == null) {
			JOptionPane.showMessageDialog(despesaUI, "Despesa não encontrada.", "Registro inexistente",
					JOptionPane.WARNING_MESSAGE);
	...
	private boolean isCamposValidos() {
		...
		if (nomeDespesa.equals("")) {
			JOptionPane.showMessageDialog(despesaUI, "Preencha o campo despesa.", "Campo obrigatório",
					JOptionPane.INFORMATION_MESSAGE);
			return false;
		}

		if (nomeTipo.equals("")) {
			JOptionPane.showMessageDialog(despesaUI, "Selecione um tipo.", "Campo obrigatório",
					JOptionPane.INFORMATION_MESSAGE);
			return false;
		} else {
			try {
		...
		if (valorDespesa.equals("")) {
			JOptionPane.showMessageDialog(despesaUI, "Preencha o campo valor.", "Campo obrigatório",
					JOptionPane.INFORMATION_MESSAGE);
			return false;
		} else {
			try {
