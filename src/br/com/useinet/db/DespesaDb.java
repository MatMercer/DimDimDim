...
public class DespesaDb extends DbClass {
	...
	public Object listaObjeto(String nome, Tipo tipo) {
		try {
			return sessao.createCriteria(Despesa.class).add(Restrictions.eq("nome", nome)).add(
					Restrictions.eq("tipo", tipo)).uniqueResult();
	...
	public Long criaObjeto(String nome, Tipo tipo, String valor) {

		despesa = new Despesa();

		setObjeto(nome, tipo, valor);
	...
	public boolean atualizaObjeto(Long despesaId, String nome, Tipo tipo, String valor) {
	...
	private void setObjeto(String nome, Tipo tipo, String valor) {
